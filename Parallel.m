...
for n=1:dotnumber
    ...
    %variable step size strategy
        ...
        [~,pos]=min(sum_e_all); %step 13
                        
%Adaptive algorithms (normalised frequency-domain block FxLMS [1], frequency-domain partitioned block FxLMS [2], and subband adaptive filters [3]) 
%can be used to obtain the update of the control filter 'delta_w'.
        for j=1:J
            for i=1:I
                if pos==Q+2 %if select null filter, additional initialisation
                    delta_w=(1-beta).*delta_w;
                    power_x=zeros(1,I);
                    power_e=zeros(1,M);
                    power=ones(I,B);
                end
                w{j,i}=w_all{j,i}(:,pos); %step 14
                w{j,i}=w{j,i}+delta_w; %step 15
                w_all{j,i}(:,Q+1)=w{j,i};%step 16
            end
        end    
    end
