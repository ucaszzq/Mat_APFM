...
function [w,e,power,mu_save,pos_save] = Parallel(I,J,M,Q,B,G,dotnumber,x,d,w_winner,S,mu_f,beta,threshold,delta)
...
for i=1:I
    for j=1:J
        for k=1:K
            vector_r{i,j,k}=zeros(G,1);%ÂË²¨²Î¿¼ÐÅºÅ
        end
    end
end
for k=1:M
...
for n=1:dotnumber
    for i=1:I
        vector_x_reverse(i,:)=[x(i,n),vector_x_reverse(i,1:end-1)]; %step 1 & step 7_1
    end
    ...
    for j=1:J
        ...
        for i=1:I
             ...
             for q=1:Q+1
                y_temp_old(1,q)=y_temp_old(1,q)+vector_x_reverse(i,:)*w_old{j,i}(:,q); %step 9_1
             end
        end
        y(j)=y_temp;
        ...
        for q=1:Q+1
            ...
            vector_y_old{j,q}=[y_old(j,q);vector_y_old{j,q}(1:end-1)]; %step 9_2
        end                
    end
    ...
    for k=1:M
        ...
        vector_e{k}(counter_i)=e{k,1}(1,n);  %step 7_2       
        for q=1:Q+1
            e_old{k,q}(1,n)=d{k,1}(1,n)+e_temp_old(1,q); %step 10 
            vector_e_old{k,q}(counter_i)=e_old{k,q}(1,n); %step 11_1
        end      
    end
    ...
    for i=1:I
        for j=1:J
            for k=1:M
                ...
                vector_r{ii,jj,kk}=[ xijk; vector_r{ii,jj,kk}(1:end-1)];  %step 6 & step 7_3
            end
        end
    end  
    ...
    %variable step size strategy
        ...
        for q=1:Q+2
            sum_ek=0;
            for k=1:M
                sum_ek=sum_ek+sum(vector_e_all{k,q}.^2); %step 11_2
            end
